...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@Repthon"))
        await sbb_b(JoinChannelRequest("@roger21v"))
        await sbb_b(JoinChannelRequest("@Repthon_vars"))
        await sbb_b(JoinChannelRequest("@Repthon_cklaish"))
        await sbb_b(JoinChannelRequest("@Repthon_support"))
        await sbb_b(JoinChannelRequest("@Repthon_help"))
        await sbb_b(JoinChannelRequest("@ZQ_LO"))
        await sbb_b(JoinChannelRequest("@Repthon_up"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                ...
                caption="**تم تشغيل سورس ريبثون بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/IMG_20220821_230957_726.jpg")
        _, groupid = await create_supergroup(
            "مجموعة اشعارات ريبثون", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/IMG_20220821_170831_450.jpg")
        _, groupid = await create_supergroup(
