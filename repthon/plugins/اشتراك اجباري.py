    ...
    if gvarstatus ("subprivate"):
        if event.is_private:
            try:
                ...
                ch = gvarstatus ("pchan")
                if not ch:
                    return await sbb_b.tgbot.send_message(BOTLOG_CHATID, "** انت لم تضع قناة الاشتراك الاجباري قم بوضعها**")
                try:
                    ch = int(ch)
                except BaseException as r:
                    return await sbb_b.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ \n{r}**")
                ...
                if "error_code" in reqt:
                    await sbb_b.tgbot.send_message(BOTLOG_CHATID, f"**حدث خطأ غير معروف قم باعادة توجيه الرسالة ل@ZQ_lO لحل المشكلة\n{reqt}**")
