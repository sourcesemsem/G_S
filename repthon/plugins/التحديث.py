...
from repthon import HEROKU_APP, UPSTREAM_REPO_URL, sbb_b
...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس ريبثون " + "**• حدث خطأ ما :**\n"
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**• سورس ريبثون محدث الى أخر اصدار❤️**"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس ريبثون"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس ريبثون أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/rogerpq/E_7_V"
    os.chdir("/app")
    try:
